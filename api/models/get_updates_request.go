...
// GetUpdatesRequest represents `getUpdates` query params.
type GetUpdatesRequest struct {
	// Optional. Identifier of the first update to be returned.
	// Must be greater by one than the highest among the identifiers
	// of previously received updates. By default, updates starting with the earliest
	// unconfirmed update are returned. An update is considered confirmed as soon
	// as getUpdates (https://core.telegram.org/bots/api#getupdates) is called with
	// an offset higher than its update_id. The negative offset can be specified to retrieve
	...
	Timeout query.ParamInt `query:"timeout,omitempty"`
